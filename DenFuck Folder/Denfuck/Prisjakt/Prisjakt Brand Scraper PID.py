...
# Define the API endpoint URL for product search
base_url = "https://www.prisjakt.nu/_internal/bff"  # Replace with actual endpoint if different
...
def fetch_all_products_by_brand(brand_id, products_total):
    all_product_ids = []
    items_per_page = 50  # Example limit per page (adjust if API allows more)
    current_offset = 0

    while current_offset < products_total:
        # Query with offset for pagination
        payload = {
            "query": """
                query getBrandProducts($brandId: ID!, $offset: Int!, $limit: Int!) {
                    brand(id: $brandId) {
                        productsTotal
            ...
            """,
            "variables": {
                "brandId": str(brand_id),  # Ensure brand ID is passed as a string
                "offset": current_offset,
                "limit": items_per_page
            }
        }

        # Send the request to fetch products for the current offset
        ...
        product_nodes = data['data']['brand']['products']['nodes']
        product_ids = [product['id'] for product in product_nodes]
        ...
        # Print progress
        print(f"Fetched {len(product_ids)} product IDs, offset now at {current_offset}")


    # Print total fetched product IDs
    print("Total Product IDs:", all_product_ids)
    return all_product_ids

# Usage example
brand_id = "61999"  # Belid brand ID
products_total = 2000  # Total products as per API response

# Fetch all product IDs
all_product_ids = fetch_all_products_by_brand(brand_id, products_total)